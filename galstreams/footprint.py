...
class StreamFootprint:
    def __init__(self, name, poly, frame=None):
        ...
        if not isinstance(frame, co.BaseCoordinateFrame):
            raise ValueError(
                "Input coordinate frame must be an astropy coordinates frame subclass *instance*, not a '{}'".format(
    ...
    def contains_coords(self, c):
        ...
        if c.isscalar:
            ...
            mask = [
                poly.contains_lonlat(x, y) for x, y in zip(rep.lon.degree, rep.lat.degree)
            ]
